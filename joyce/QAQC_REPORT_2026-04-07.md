...
| 3 | What if soil is located: only behind / both sides / only in front? | DONE (v0.4.0) + Toe Sprint | v0.4.0 added front soil depth y + passive credit toggle. Toe Sprint adds explicit b_toe geometry, P_toe_soil vertical load when y > t_base, lateral cover soil drawn on the toe side | Das §17.2 p.697, §17.4 p.699 |
| 4 | Diagram parts not aligned with current wall geometry (H₁/H₂/toe/heel/shear key) | DONE (Toe Sprint) | New `b_toe` input + correct stem positioning + toe/heel labels with arrows on the plot. Wall geometry block in report shows toe, stem, heel, total b breakdown explicitly | Das §17.2 Fig. 17.4 (p. 697) |
...
| 8 | (Toe Sprint) Add explicit toe input feature | DONE | New `b_toe` input field. When > 0, b_base auto-recomputes as b_toe + t_stem + b_heel and the stem is positioned correctly. When 0, legacy derivation is used (regression-safe). Stem moment arm fixed: m1 = P1·(b_toe + t_stem/2). Backfill width fixed: P3 uses b_heel directly (was b_base − t_stem). New P_toe_soil vertical load when y_front > t_base | Das §17.2 p.697 (proportioning); §17.4 p.699 (stability); §17.6 p.703 (sliding); Bowles 1996 §12.4 |
| 9 | (Toe Sprint) Add surcharge diagram on the plot | DONE | Five orange downward arrows on top of the heel + "q = X kPa" label, plus a depth-independent rectangular Pa_sur lateral pressure band overlaid on the active triangle and a Pa_sur force arrow at H/2 | Das §16.8 Lateral pressure due to surcharge (p. 665) |
| 10 | (Toe Sprint) Larger and higher-quality wall plot | DONE | Plot height bumped from 420 px → 680 px; all annotations resized to 11–13 pt; explicit y-axis range; title now cites Das §17.4 / Fig. 17.4; toe and heel labels with arrows; surcharge label added | — |
